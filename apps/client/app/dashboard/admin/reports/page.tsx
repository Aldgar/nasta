...
export default function AdminReportsPage() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<"ABUSE" | "SECURITY">(
    "ABUSE",
  );
  ...
  const fetchTickets = useCallback(async () => {
    setLoading(true);
    const params = new URLSearchParams({ scope, category: categoryFilter });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[var(--foreground)]">
            {selectedTicketId ? "Report Details" : "Reports"}
          </h1>
          {!selectedTicketId && (
            <p className="mt-1 text-sm text-[var(--muted-text)]">
              Review and manage abuse and security reports.
            </p>
          )}
        </div>
        {!selectedTicketId && (
          <span className="rounded-full bg-[var(--alert-red)]/20 px-3 py-1 text-xs font-semibold text-[var(--alert-red)]">
      ...
      {!selectedTicketId && (
        <>
          ...
          <div className="flex rounded-lg border border-[var(--border-color)] bg-[var(--surface)] overflow-hidden w-fit">
            {(
              [
                { value: "ABUSE", label: "🚩 Abuse Reports" },
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-12 text-center">
              <p className="text-[var(--muted-text)]">
                No {categoryFilter === "ABUSE" ? "abuse" : "security"} reports
                found
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {tickets.map((ticket) => (
                <div
                  ...
                  className="cursor-pointer rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 transition-colors hover:border-[var(--alert-red)]/30"
                >
          ...
          {activeModal === "respond" && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-6 shadow-xl">
                ...
                <p className="mt-1 text-xs text-[var(--muted-text)]">
                  Send a response message regarding this report.
                </p>
          ...
          {activeModal === "note" && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-6 shadow-xl">
                ...
                <p className="mt-1 text-xs text-[var(--muted-text)]">
                  This note is visible to admins only, not sent to the reporter.
                </p>
                <textarea
                  value={internalNote}
