...
export default function ServiceProviderRatingScreen() {
  const router = useRouter();
  const { applicationId, employerName } = useLocalSearchParams();
  ...
  const handleSubmit = async () => {
    if (platformRating === 0 || employerRating === 0) {
    ...
    try {
      ...
      const res = await fetch(
        `${base}/ratings/applications/${applicationId}/service-provider`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            employerComment: employerComment.trim() || undefined,
          }),
        }
      );
            ...
            <View style={styles.starsContainer}>
              {[0, 1, 2, 3, 4].map((i) => (
                <Star
                  key={i}
                  index={i}
                  filled={i < employerRating}
                  onPress={() => setEmployerRating(i + 1)}
                />
              ))}
            </View>
            <TextInput
