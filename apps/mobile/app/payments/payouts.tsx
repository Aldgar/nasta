...
const formatIban = (iban: string): string => {
  ...
  const translateCountryName = (countryCode: string): string => {
    ...
    return translated !== key ? translated : (EUROPEAN_COUNTRIES.find(c => c.code === countryCode)?.name || countryCode);
  };

  // Validate IBAN format and country code match
  const validateIban = (iban: string, countryCode: string): { valid: boolean; error?: string } => {
    const cleaned = iban.replace(/\s/g, '').toUpperCase();
    ...
    if (!cleaned.startsWith(countryCode.toUpperCase())) {
      ...
      return { valid: false, error: t("payments.ibanCountryError", { code: countryCode.toUpperCase(), country: countryName }) };
    }
        ...
        { text: t("common.cancel"), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const res = await fetch(`${baseUrl}/payments/connect/bank-account/${bankAccountId}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
              });
              ...
              if (!res.ok) {
                ...
                Alert.alert(t("common.error"), errorData.message || t("payments.failedToDeleteBankAccount"));
                return;
              }
              
              Alert.alert(t("common.success"), t("payments.bankAccountDeletedSuccessfully"));
              await fetchStatus();
            } catch (error: any) {
              console.error('Error deleting bank account:', error);
              Alert.alert(t("common.error"), t("payments.failedToDeleteBankAccountTryAgain"));
            } finally {
              setLoading(false);
            }
          }
  ...
  const handleSetDefault = async (bankAccountId: string) => {
    try {
      ...
      const res = await fetch(`${baseUrl}/payments/connect/bank-account/${bankAccountId}/set-default`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        Alert.alert(t("common.error"), errorData.message || t("payments.failedToSetDefaultBankAccount"));
        return;
      }
      ...
      Alert.alert(t("common.error"), t("payments.failedToSetDefaultBankAccountTryAgain"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitBankAccount = async () => {
    if (!accountHolderName.trim() || (!useIban && (!accountNumber.trim() || !routingNumber.trim())) || (useIban && !iban.trim())) {
    ...
    console.log('[BankAccount] Submit - Country value:', country, 'Type:', typeof country, 'Normalized:', countryCode);
    
    if (countryCode.length !== 2 || !/^[A-Z]{2}$/.test(countryCode)) {
      console.error('[BankAccount] Invalid country code format:', countryCode, 'Length:', countryCode.length);
      Alert.alert(t("common.error"), t("payments.invalidCountryCode", { country }));
      return;
    }
    ...
    if (!countryMatch) {
      ...
      console.log('[BankAccount] Available countries:', EUROPEAN_COUNTRIES.map(c => c.code).join(', '));
      Alert.alert(t("common.error"), t("payments.countryNotSupported", { country: countryCode }));
      return;
    }
    
    console.log('[BankAccount] Country validated successfully:', countryCode, 'Name:', countryMatch.name);

    // Validate IBAN if using IBAN
    if (useIban && iban.trim()) {
      const ibanValidation = validateIban(iban, countryCode);
      if (!ibanValidation.valid) {
        Alert.alert(t("payments.invalidIban"), ibanValidation.error || t("payments.checkIbanFormat"));
        return;
      }
    }
    ...
    try {
      ...
      console.log('[BankAccount] Submitting request body:', JSON.stringify(requestBody, null, 2));
      console.log('[BankAccount] Country in request:', requestBody.country, 'Type:', typeof requestBody.country);
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (errorMessage.includes('country') || errorMessage.includes('Country')) {
            ...
            if (errorMessage.includes('Invalid country code') || errorMessage.includes('2-letter ISO')) {
          ...
          } else if (errorMessage.includes('IBAN') || errorMessage.includes('iban')) {
            errorMessage = t("payments.invalidIbanFormat");
          } else if (errorMessage.includes('routing') || errorMessage.includes('account number')) {
          ...
          console.error('[BankAccount] Failed to parse error response:', parseError);
        ...
        console.error('[BankAccount] Final error message to show user:', errorMessage);
        ...
        {loading ? (
            ...
            <>
                <View style={[styles.statusCard, { backgroundColor: isDark ? 'rgba(12, 22, 42, 0.82)' : '#fff' }]}>
                    ...
                    <View style={[styles.badge, status?.isEnabled ? styles.bgGreen : styles.bgOrange]}>
                        <Text style={[styles.badgeText, status?.isEnabled ? styles.textGreen : styles.textOrange]}>
                            {status?.isEnabled ? t("payments.active") : t("payments.restricted")}
                        </Text>
                    </View>
                </View>
                ...
                {!status?.isEnabled && status?.requirements && (
                  <View style={[styles.requirementsCard, { backgroundColor: isDark ? 'rgba(251, 191, 36, 0.1)' : '#fef3c7', borderColor: isDark ? 'rgba(251, 191, 36, 0.3)' : '#fbbf24' }]}>
                    <View style={styles.requirementsHeader}>
                      <Ionicons name="information-circle" size={20} color={isDark ? '#fbbf24' : '#d97706'} />
                      <Text style={[styles.requirementsTitle, { color: isDark ? '#fbbf24' : '#d97706' }]}>
                        {t("payments.accountVerificationRequired")}
                      </Text>
                    </View>
                    <Text style={[styles.requirementsText, { color: isDark ? '#fde68a' : '#92400e' }]}>
                    ...
                    {(status.requirements.currentlyDue && status.requirements.currentlyDue.length > 0) && (
                      <View style={styles.requirementsList}>
                        <Text style={[styles.requirementsSubtitle, { color: isDark ? '#fbbf24' : '#d97706' }]}>
                        ...
                        {status.requirements.currentlyDue
                          .filter((req: string) => {
                            ...
                            return !req.startsWith('business_profile.') && 
                          ...
                          .map((req: string, idx: number) => {
                            ...
                            const friendlyLabels: { [key: string]: string } = {
                              ...
                              'individual.address.line1': t("payments.streetAddress"),
                              'individual.address.postal_code': t("payments.postalCode"),
                              'individual.dob.day': t("payments.dateOfBirthDay"),
                              'individual.dob.month': t("payments.dateOfBirthMonth"),
                              'individual.dob.year': t("payments.dateOfBirthYear"),
                              'individual.phone': t("profile.phone"),
                            };
                            
                            const label = friendlyLabels[req] || req.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase());
                            
                            return (
                              <Text key={idx} style={[styles.requirementItem, { color: isDark ? '#fde68a' : '#92400e' }]}>
                                • {label}
                              </Text>
                            );
                          })}
                    ...
                    <Text style={[styles.requirementsNote, { color: isDark ? '#fde68a' : '#92400e' }]}>
                        ...
                        <View style={styles.bankAccountHeader}>
                          ...
                          <View style={styles.bankAccountInfo}>
                            ...
                            {bankAccount.defaultForCurrency && (
                              <View style={[styles.defaultBadge, { backgroundColor: isDark ? 'rgba(34, 197, 94, 0.2)' : '#d1fae5' }]}>
                                <Text style={[styles.defaultBadgeText, { color: isDark ? '#6ee7b7' : '#065f46' }]}>
                                  {t("payments.default")}
                                </Text>
                              </View>
                            )}
                        ...
                        <View style={[styles.bankAccountActions, { borderTopColor: isDark ? 'rgba(201,150,63,0.12)' : '#F0E8D5' }]}>
                          {!bankAccount.defaultForCurrency && (
                            <TouchableOpacity
                              ...
                              disabled={loading}
                            >
                              <Ionicons name="star" size={16} color={isDark ? '#6ee7b7' : '#065f46'} />
                              <Text style={[styles.bankAccountActionText, { color: isDark ? '#6ee7b7' : '#065f46' }]}>
                          ...
                          {status.bankAccounts.length > 1 && (
                            <TouchableOpacity
                              style={[styles.bankAccountActionBtn, { backgroundColor: isDark ? 'rgba(239, 68, 68, 0.2)' : '#fee2e2' }]}
                              onPress={() => handleDeleteBankAccount(bankAccount.id)}
                              disabled={loading}
                            >
                              <Ionicons name="trash-outline" size={16} color={isDark ? '#f87171' : '#dc2626'} />
                              <Text style={[styles.bankAccountActionText, { color: isDark ? '#f87171' : '#dc2626' }]}>
                                {t("common.delete")}
                              </Text>
                            </TouchableOpacity>
                          )}
                ...
                >
                    <Text style={styles.btnText}>
                        {status?.bankAccounts && status.bankAccounts.length > 0 ? t("payments.addAnotherBankAccount") : t("payments.addBankAccount")}
                    </Text>
                    <Ionicons name={status?.bankAccounts && status.bankAccounts.length > 0 ? "add-circle-outline" : "add-circle-outline"} size={20} color="white" />
                </TouchableOpacity>
                        ...
                        <View style={[styles.modalContent, { backgroundColor: isDark ? 'rgba(12, 22, 42, 0.90)' : '#ffffff' }]}>
                            ...
                            <ScrollView style={styles.formContainer} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                                ...
                                <View style={styles.toggleContainer}>
                                    <TouchableOpacity
                                        style={[styles.toggleBtn, useIban && styles.toggleBtnActive]}
                                        onPress={() => setUseIban(true)}
                                    >
                                        <Text style={[styles.toggleText, useIban && styles.toggleTextActive]}>{t("payments.ibanEu")}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[styles.toggleBtn, !useIban && styles.toggleBtnActive]}
                                        onPress={() => setUseIban(false)}
                                    >
                                        <Text style={[styles.toggleText, !useIban && styles.toggleTextActive]}>{t("payments.accountAndRouting")}</Text>
                                    </TouchableOpacity>
                                </View>
                                ...
                                >
                                    <Text style={{ color: colors.text, fontSize: 16 }}>
                                        {country ? translateCountryName(country) : t("profile.selectCountry")}
                                    </Text>
                                                ...
                                                {EUROPEAN_COUNTRIES.map((countryOption) => (
                                                    <TouchableOpacity
                                                        ...
                                                        onPress={() => {
                                                            ...
                                                            console.log('[BankAccount] Country picker - Selected:', countryOption.name, 'Code:', selectedCode, 'Type:', typeof selectedCode);
                                                            
                                                            // Ensure we're setting a valid 2-letter code
                                                            if (selectedCode && selectedCode.length === 2 && /^[A-Z]{2}$/.test(selectedCode)) {
                                                                setCountry(selectedCode);
                                                                console.log('[BankAccount] Country state updated to:', selectedCode);
                                                            } else {
                                                                console.error('[BankAccount] Invalid country code from picker:', selectedCode);
                                                                Alert.alert(t("common.error"), t("payments.invalidCountrySelection"));
                                                                return;
                                                            }
                                                            ...
                                                            if (iban && !iban.replace(/\s/g, '').toUpperCase().startsWith(selectedCode)) {
                                                                setIban('');
                                                            }
                                                    ...
                                                    >
                                                        <Text style={[
                                                            ...
                                                            { color: colors.text },
                                                            country === countryOption.code && styles.pickerItemTextSelected
                                                        ]}>
                                ...
                                >
                                    {saving ? (
                                        ...
                                        <Text style={styles.submitBtnText}>{t("payments.addBankAccount")}</Text>
                                    )}
                                </TouchableOpacity>
                            </ScrollView>
                        </View>
